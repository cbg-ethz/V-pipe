...
def input_align(wildcards):
    list_output = []
    list_output.append(os.path.join(wildcards.dataset, "preprocessed_data/R1.fastq"))
    if config.input['paired']:
        list_output.append(os.path.join(wildcards.dataset, "preprocessed_data/R2.fastq"))
    return(list_output)
...
if config.general["aligner"] == "bwa":
    def input_align_gz(wildcards):
        ...
        if config.input['paired']:
            list_output.append(os.path.join(wildcards.dataset, "preprocessed_data/R2.fastq.gz"))
        return(list_output)
    ...
    rule bwa_align:
        ...
        output:
            REF = temp(os.path.join(config.general["temp_prefix"], "{dataset}/alignments/REF_aln.sam")),
            TMP_SAM = temp(os.path.join(config.general["temp_prefix"], "{dataset}/alignments/tmp_aln.sam")),
        params:
    ...
    if config.input['paired']:
        rule bowtie_align:
            input:
                ...
                INDEX6 = "{}.rev.2.bt2".format(reference_file)
            output:
                temp(os.path.join(config.general["temp_prefix"], "{dataset}/alignments/REF_aln.sam")),
            params:
