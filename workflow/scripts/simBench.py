...
def parse_args():
    ...
    parser.add_argument(
        ...
        dest="num_haplotypes",
        type=int,
        help="Number of underlying haplotypes",
    )
    parser.add_argument(
        "-c",
        required=False,
    ...
    parser.add_argument(
        ...
        dest="read_length",
        type=int,
        help="Length (in base pairs) of simulated reads",
    )
    parser.add_argument(
        "-p",
        required=False,
    ...
    parser.add_argument(
        ...
        dest="freq_dstr",
        type=str,
        choices=["unif", "geom", "dirichlet", "cust"],
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="INT",
        dest="deletion_length",
        type=int,
        help="Deletion length in bp",
    )
    parser.add_argument(
        "-q",
        required=False,
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="tree_like",
        help="Indicate whether to generate haplotype by "
    ...
    parser.add_argument(
        ...
        dest="output",
        type=str,
        choices=["master", "haplotypes", "reads", "all"],
    ...
    if noFR or del_len:
        ...
        for i in range(num_deletions):
            if del_len is None:
                ...
                del_len_cur = del_len
            del_locus = np.random.randint(0, haplotype.size - del_len_cur, size=1)[0]
            if verbose:
                del_out = (
        ...
        for i in range(num_insertions):
            ins_locus = np.random.randint(0, haplotype.size - 1, size=1)
            ...
            if verbose:
                ...
                print(f"{ins_locus[0] - 1}:ins-{ins_seq.tostring().decode()}")
    else:
        # Draw positions at random where to introduce deletions
        del_loci = np.random.randint(0, haplotype.size - 1, size=num_deletions)
        if verbose:
        ...
        for i in range(num_insertions):
            ...
            if verbose:
                # print the locus (0-indexing) after which the insertion is
                # placed and the sequence
                print(f"{ins_locus[0] - 1}:ins-{ins_seq[0].decode()}")
...
def leaf(
    ...
    if tree.depth < tree.max_depth:
        ...
        tree.children = [
            Tree(sequences[0], tree.max_depth, tree),
            Tree(sequences[1], tree.max_depth, tree),
        ]
        for child in tree.children:
            leaf(child, mutation_rate, del_rate, ins_rate, noFR, del_len, seed, verbose)
    else:
        return
...
def print_params(mutation_rate, deletion_rate, insertion_rate):
    ...
    if insertion_rate >= 1:
        print(f"Number of insertions per haplotype: " f"{int(insertion_rate)}")
    else:
...
def main():
    ...
    seed = args.seed if args.seed is not None else np.random.randint(1, 1000, size=1)[0]
    ...
    if args.output == "master":
        ...
        if args.haplotype_seqs is None:
            ...
            if args.use_master:
                ...
                if args.verbose:
                    print_params(
                        args.mutation_rate, args.deletion_rate, args.insertion_rate
                    )
                haplotype_seqs = [
                    mutate(
                        haplotype_seq,
                ...
                np.random.seed(seed)
                idxs = np.random.randint(0, len(sequences) - 1, size=num_haplotypes)
                ...
                haplotype_seqs = [
                    sim_haplotypes(length=args.genome_length, weights=weights, s=seed)
            ...
            if args.use_master:
                ...
                output_file = os.path.join(
                    outdir_haps, "".join(("haplotype", str(idx - 1), ".fasta"))
                )
                with open(output_file, "w") as outfile:
    ...
    if args.output == "reads" or args.output == "all":
        fragment_mean, fragment_sd = [int(x) for x in args.fragment_size.split(",")]
        ...
        if args.freq_dstr == "unif":
            ...
            # Rename output file
            if args.paired:
                os.rename("".join((outprefix, "1.fq")), "".join((outprefix, "1.fastq")))
                os.rename("".join((outprefix, "2.fq")), "".join((outprefix, "2.fastq")))
            else:
                os.rename("".join((outprefix, ".fq")), "".join((outprefix, "1.fastq")))
            ...
            if args.freq_dstr == "geom":
                ...
                # write to output
                fasta_record = collections.namedtuple("fasta_record", "id seq")
                output_file = os.path.join(outdir_haps, "haplotype_frequencies.fasta")
                with open(output_file, "w") as outfile:
                    for i in range(num_haplotypes):
            ...
            for idx in range(num_haplotypes):
                ...
                print(
                    f"Reading file containing sequence of haplotype {idx} "
                    f"from {haplotype_file}"
                )
                # remove true deletions (if present) before generating reads
            ...
            if args.paired:
                sh.cat(
                    outfiles_R1, _out=os.path.join(outdir_reads, "simreads_R1.fastq")
                )
                sh.cat(
                    outfiles_R2, _out=os.path.join(outdir_reads, "simreads_R2.fastq")
                )
                for idx in range(len(outfiles_R1)):
                    os.remove(outfiles_R1[idx])
                    os.remove(outfiles_R2[idx])
            else:
                sh.cat(outfiles, _out=os.path.join(outdir_reads, "simreads_R1.fastq"))
                for f in outfiles:
                    os.remove(f)
