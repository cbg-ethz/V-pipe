        ...
        for record in self.__vcf.records:
                ...
                if self.__gb.feature.type in self.__annotated_features:
                    ...
                    for k, v in qualifiers.items():
                        if v in feature.qualifiers:
                            ...
                            record.INFO[k] = (
                                feature.qualifiers[v][0]
                                .replace(";", "[semi-colon]")
                                .replace(",", "[comma]")
                                .replace(" ", "[space]")
                            )
                            if v == "anticodon":
                                record.INFO[k] = "anticodon{0}".format(record.INFO[k])
            ...
            if record.is_indel:
                ...
                if int(record.INFO["IsGenic"]):
                    ...
                    record.INFO["RefCodon"][record.INFO["SNPCodonPosition"]] = ref_base
                    ...
                    if feature.strand == -1:
                        ...
                        record.INFO["Comments"] = "Negative:{0}->{1}".format(
                            Seq(record.REF).complement(), alt_base
                        )
...
class GenBank(object):
    ...
    def codon_by_position(self, pos):
        ...
        return [
            self.gene_codons[self._accession][self._index][codon_position],
            gene_position % 3,
            codon_position + 1,
        ]
