...
rule aggregate_diversity:
    input:
        fnames_diversity=expand('{dataset}/variants/SNVs/diversity_measures.csv', dataset=datasets),
        fnames_shannon=expand('{dataset}/variants/SNVs/position_shannon_entropy.csv', dataset=datasets),
    output:
        diversity_csv=os.path.join(config.output["datadir"], "aggregated_diversity.csv"),
        shannon_csv=os.path.join(config.output["datadir"], "aggregated_entropy.csv"),
    conda:
        config.diversity["conda"]
    script:
