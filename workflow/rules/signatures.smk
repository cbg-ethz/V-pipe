...
rule covvfit:
    ...
    shell:
        """
        {params.COVVFIT} infer -i "{input.deconvoluted}" -o "{output.outdir}" -c "{input.covvfit_conf}" --max-days {params.max_days} --horizon {params.horizon} 2> >(tee -a {log.errfile} >&2) > >(tee -a {log.outfile})
        """
